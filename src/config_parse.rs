...
pub fn config_parse_descriptor(
    ...
    if let Some((fn_name, args)) = parse_fn(s) {
        ...
        let actions = [
            ...
            ("overloadi2", Op::OverloadIdleTimeout2, vec!["descriptor", "descriptor", "timeout"]),
        ...
        for (name, op, arg_types) in actions {
            if fn_name == name {
                ...
                let data = match op {
                    ...
                    Op::OverloadIdleTimeout | Op::OverloadIdleTimeout2 => DescriptorData::OverloadIdle(DescOverloadIdle { action1_idx: parsed_args[0], action2_idx: parsed_args[1], timeout: parsed_args[2] as u16 }),
