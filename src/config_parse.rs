...
pub fn config_parse_descriptor(
    ...
    if let Some((fn_name, args)) = parse_fn(s) {
        ...
        let actions = [
            ...
            ("overloadr", Op::OverloadReleaseGap, vec!["layer", "descriptor", "timeout", "timeout"]),
        ...
        for (name, op, arg_types) in actions {
            if fn_name == name {
                ...
                let data = match op {
                    ...
                    Op::OverloadReleaseGap => DescriptorData::OverloadRelease(DescOverloadRelease { layer_idx: parsed_args[0], action_idx: parsed_args[1], tap_timeout: parsed_args[2] as u16, following_timeout: parsed_args[3] as u16 }),
