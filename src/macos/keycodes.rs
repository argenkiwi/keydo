// CGKeyCode ↔ KEYD code translation.
// Port of keyd/src/macos/keycodes.c

use crate::keys::*;

// Forward table: CGKeyCode (index 0–127) → KEYD code. 0 = unmapped.
static CGKEY_TO_KEYD: [u16; 128] = [
...
pub fn cgkey_to_keyd(cgkey: u16) -> u16 {
    if cgkey >= 128 { 0 } else { CGKEY_TO_KEYD[cgkey as usize] }
}

pub fn keyd_to_cgkey(keyd_code: u16) -> Option<u16> {
    if keyd_code == 0 {
        return None;
    }
    CGKEY_TO_KEYD
        .iter()
        .position(|&k| k == keyd_code)
        .map(|i| i as u16)
}
